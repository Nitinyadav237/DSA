...
function findAllPaths(maze: number[][]): string[]{
    const n = maze.length
    const dir: [number, number][] = [[1, 0], [0, 1], [-1, 0], [0, -1]] //d R U L
    const paths: string[] = []
    ...
    function solveMaze(row: number, col: number): void {
        //basecase
        if (row === n - 1 && col === n - 1) {
...
function findPaths(maze: number[][]): String[]{
    ...
    const path:number[][]= Array.from({length:n},()=>Array(n).fill(0))
