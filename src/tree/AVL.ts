class TreeNodeAVL<T>{
    value: T;
    leftNode: TreeNodeAVL<T> | null;
    rightNode: TreeNodeAVL<T> | null;
    height: number;
...
class AVL<T>{
    root: TreeNodeAVL<T> | null;
    constructor() {
        this.root = null;
    }

    //height 
    private height(node:TreeNodeAVL<T>|null):number {
        return node ? node.height : 0
    }
    //balance
    private getBalance(node: TreeNodeAVL<T>) {
...
// T1  T2
    ...
    private rightRotation(y: TreeNodeAVL<T>): TreeNodeAVL<T>{
        //initial value
        const x = y.leftNode
    ...
    //          / \
    //         T2 T3
    private leftRotation(x: TreeNodeAVL<T>): TreeNodeAVL<T>{
        const y = x.rightNode
        const T2 = y!.leftNode
    ...
    private insertNode(node: TreeNodeAVL<T> | null, value: T): TreeNodeAVL<T> {
    ...
    private minValue(node: TreeNodeAVL<T>):T {
        let min = node.value
    ...
    private deleteNode(node: TreeNodeAVL<T> | null, value: T): TreeNodeAVL<T> | null{
    ...
    inorderTraversalAVL(): T[] {
        const result: T[] = []
        this.inorderHelper(this.root,result)
        return result
    }
    private inorderHelper(node: TreeNodeAVL<T> | null, result: T[]) :void{
        if (node !== null) {
    ...
    search(value: T): TreeNodeAVL<T> | null{
        return this.searchNode(this.root,value)
    }
    private searchNode(node: TreeNodeAVL<T> | null, value: T): TreeNodeAVL<T> | null{
        if (node === null) return null;
        if (value < node.value) return this.searchNode(node.leftNode, value);
        else if (value > node.value) return this.searchNode(node.rightNode, value)
        else return node
    }
